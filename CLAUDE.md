...
Dependencies are managed via **vcpkg**, bootstrapped automatically by the top-level `CMakeLists.txt`: if `VCPKG_ROOT` is unset, it clones vcpkg into `.vcpkg/`, **pins the checkout to `VCPKG_PINNED_COMMIT`** (deliberately the same commit as the `builtin-baseline` in `vcpkg.json`, so tool and ports come from one audited revision — a supply-chain measure; never reintroduce an auto-`git pull` here), and bootstraps it before `project()` is even declared; the dependencies declared in `vcpkg.json` (GTest, fmt, spdlog) are then installed by vcpkg's manifest-mode resolution during configure. The *first* configure can take a while and requires network/git access; later configures only re-checkout if the pin changed. **To bump vcpkg**: update the `builtin-baseline` in `vcpkg.json` AND `VCPKG_PINNED_COMMIT` in `CMakeLists.txt` to the same new commit, then re-configure. A user-set `VCPKG_ROOT` env checkout is treated as user-managed: no git operations are performed on it.
...
- **`src/`** — holds no sources of its own; `src/CMakeLists.txt` only aggregates the two real targets below. There is no application executable: the deliverable is the DLL that IIS loads.
  - `src/core/` is the IIS/Windows-independent layer: static library target `IISNativeModuleCore` (namespace `core`), exporting its headers as `<core/...>` via a PUBLIC include dir. It holds `Verdict` (the platform-free mirror of `REQUEST_NOTIFICATION_STATUS`), the C++20 **concepts** `Request`/`Response`/`Context`/`EventProvider` mirroring their IIS counterparts, `AsciiToLower` (locale-independent folding — never use `std::towlower` for URL matching), and `ModuleHandler` — the actual request logic (403 + reason `Forbidden by IISRequestLevelModule` for cooked URL paths ending in `/forbidden`, `X-IISRequestLevelModule: 1` header otherwise). Nothing in this directory may include Windows or IIS headers; HRESULT-shaped values travel as `std::int32_t`.
    - **The layer is compile-time polymorphic, and deliberately so** (measured: see below). `ModuleHandler::OnBeginRequest` is a template constrained by the concepts, so adapters and mocks bind with no vtable and no allocation. It is the one thing that **must** live in `ModuleHandler.h`: a template has to be visible where it is instantiated, and it is instantiated with the IIS adapter types in `src/module`, which core must not know about. Everything not type-dependent — the `EndsWithForbidden` matching algorithm — lives in `ModuleHandler.cpp`, per the project's one-`.h`/`.cpp`-pair-per-class convention.
    - `core::RawHttpRequest::absPath` is a **non-owning `std::wstring_view`** over the server's buffer, and `Response`'s `string_view` parameters carry a **null-termination precondition** (the IIS adapter forwards `.data()` straight to IIS rather than materializing a `std::string`); every call site passes views over string literals. Do not reintroduce copies here — that is where the cost was.
  - `src/module/` is the IIS native module itself: target `IISNativeModule`, a `SHARED` library (DLL) that exports `RegisterModule` via `iis_native_module.def` and links `IISNativeModuleCore`. Files here — as in `src/core` and `test/core` — are named after the class they hold (`IISRequestLevelModule.h/.cpp` etc., PascalCase). The four adapters `IISContext`/`IISRequest`/`IISResponse`/`IISEventProvider` are each a `.h`/`.cpp` pair and each carries a `static_assert` proving it satisfies its core concept — with no base class to inherit from, that assert is the only place a broken contract surfaces, so keep it. `iis::IISRequestLevelModule` is a thin adapter: its constructor takes a non-owning `const core::ModuleHandler*`, and `OnBeginRequest` wraps the live IIS objects in the adapters, delegates, and maps the returned `Verdict` back to a `REQUEST_NOTIFICATION_STATUS`. `iis::IISRequestLevelModuleFactory` owns the shared (stateless, `const`) `core::ModuleHandler` and injects it into every instance it creates; `register_module.cpp` subscribes to `RQ_BEGIN_REQUEST` only — add further `RQ_*` flags there as methods gain implementations.
- **`test/`** — GTest-based tests, target/project `IISNativeModuleTest`. Globs `test/*.cpp` and `test/core/*.cpp`, and links `IISNativeModuleCore`. `test/main.cpp` is a standard GTest `main()` running `RUN_ALL_TESTS()`. Mirror the `src/` layout under `test/` for new tests (one GTest `TEST` per behavior, wrapped in the same namespace as the code under test). `test/core/ModuleHandlerTest.cpp` exercises `core::ModuleHandler` through hand-rolled mocks — the pattern to follow for testing module logic without IIS. The mocks satisfy the core concepts **by shape alone**: no base class, no `override`, no vtable, each confirmed by a `static_assert`.
...
- **Why core is templates rather than virtual interfaces**: measured on branch `perf/dispatch-microbenchmark` (`bench/`, Release, 2M iterations x 7 reps, generic path, min ns per call including per-request adapter construction). Virtual interfaces copying into `std::wstring`/`std::string`: **139.00 ns**. Same interfaces, zero-copy + `AsciiToLower`: **22.49 ns**. Templates + zero-copy + `AsciiToLower`: **9.98 ns** — faster than the pre-refactor direct implementation (10.86 ns), because inline ASCII folding beats the `_wcsnicmp` CRT call. Copies cost ~116 ns, virtual dispatch ~4–13 ns: **the allocations were the story, dispatch was the tiebreaker.** Don't reintroduce either. Note the 9.98 ns figure came from a fully-inlined variant; here the adapters and `EndsWithForbidden` are in `.cpp` files, so cross-TU calls remain unless `/GL`+`/LTCG` is enabled — the copies and the vtable, which dominated, are gone either way.
- All targets compile with `/guard:cf` (Control Flow Guard, also passed at link) and, under MSVC, `/sdl` — hardening for a DLL that runs network-facing inside w3wp.exe. `IISRequestLevelModule::OnBeginRequest` wraps its body in `try/catch(...)`: IIS notification methods must never let a C++ exception escape (it would crash the worker process), so keep that boundary when adding new notifications.
- CI actions in `.github/workflows/ci.yml` are pinned by commit SHA (supply-chain); Dependabot (`.github/dependabot.yml`) proposes pin bumps. When adding an action, pin it by SHA with a `# vX` comment.
