...
- **`src/`** — the main application (`CMakeLists.txt` defines target `IISNativeModuleApp`, project `IISNativeModuleApp`). Sources are glob-collected (`*.cpp`/`*.h` in the directory), so **new files under `src/` are picked up automatically at the next CMake configure** — no need to edit `src/CMakeLists.txt` when adding a file there.
  ...
  - `src/module/` is the IIS native module itself: target `IISNativeModule`, a `SHARED` library (DLL) that exports `RegisterModule` via `iis_native_module.def`. `iis::IISRequestLevelModule` (`iis_request_level_module.h/.cpp`) implements only `OnBeginRequest` (plus the mandatory `Dispose()`): requests whose cooked URL path ends with `/forbidden` get a 403 with reason `Forbidden by IISRequestLevelModule`, everything else gets an `X-IISRequestLevelModule: 1` response header. `iis::IISRequestLevelModuleFactory` creates instances; `register_module.cpp` subscribes to `RQ_BEGIN_REQUEST` only — add further `RQ_*` flags there as methods gain implementations.
